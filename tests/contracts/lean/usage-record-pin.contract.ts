 ...
 * fixture text and the census script to the same predicate; the pending test binds B2a's converter to the pin. Round 3 of B0
 * (review-r2 should-fix 7): case is folded OUTSIDE single-quoted literals only, so 'usage_live' -> 'USAGE_LIVE' (a different SQL
 * predicate) changes the pin while OR -> or (the same predicate) does not; the pinned value is unchanged because the literals in the
 * pinned text are already lower case.
 */
...
/** sha256 of the predicate after comment lines are dropped, whitespace is collapsed (none inside the outer parentheses) and case is folded outside single-quoted literals. Identical in plimsoll-cloud. */
export const USAGE_RECORD_PREDICATE_PIN = "e075f3989cd668f4938c4964b6634c3b20bbab1604bfc623bc1164a195f95707";
/** Lower-cases SQL keywords and identifiers but never the contents of a '...' literal (round 3: a literal's case is part of the predicate). */
export const foldCaseOutsideLiterals = (text: string) => text.split("'").map((part, index) => (index % 2 === 0 ? part.toLowerCase() : part)).join("'");
export const normalisePredicate = (text: string) =>
  foldCaseOutsideLiterals(text.split("\n").filter((line) => !line.trim().startsWith("--")).join(" ").replace(/\s+/g, " ").replace(/\(\s+/g, "(").replace(/\s+\)/g, ")").trim());
...
test("<UR> pin guard (round 3): a case change inside a literal ('usage_live' -> 'USAGE_LIVE', a different predicate) changes the pin; a keyword case change (OR -> or, the same predicate) does not", () => {
  const text = fixture();
  assert.ok(text.includes("'usage_live'") && text.includes(" OR "), "the fixture carries the literal and the keyword this guard flips");
  assert.notEqual(pinOf(text.replace("'usage_live'", "'USAGE_LIVE'")), USAGE_RECORD_PREDICATE_PIN, "a literal's case is part of the predicate");
  assert.equal(pinOf(text.replace(/ OR /g, " or ")), USAGE_RECORD_PREDICATE_PIN, "keyword case is not");
  assert.equal(normalisePredicate("A OR 'Mixed Case' AND b"), "a or 'Mixed Case' and b");
});
