...
- `pnpm proof:outbox`: 35 deterministic checks pass using temporary SQLite files and fake `fetch`; provider network and live state are not touched.
...
- Pending envelopes are never auto-evicted. Backlog pressure or an undersized migration slice pauses only legacy migration; atomic new configured capture continues and reports degraded state.
- A legacy row larger than the configured migration slice but within the valid item ceiling remains pending with `migration_slice_budget` / `slice_budget_too_small`; raising the slice budget resumes it. It is never quarantined merely because of a maintenance budget.
