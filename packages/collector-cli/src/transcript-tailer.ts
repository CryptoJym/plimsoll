...
export class TranscriptTailer {
   ...
   * every transcript under the capture roots, with whether this tailer's
  ...
  private async runScan(options: TranscriptScanOptions): Promise<TranscriptScanResult> {
    ...
    for (let index = 0; index < discoveredFiles.length; index += 1) {
      ...
      if (automatic?.phase === "capture") {
        ...
        if (decision.decision === "exclude") {
          const growthStart = captureBaselinePostEnrollmentOffset(this.buffer.database, "claude_code", observation);
    ...
    let queue = (automatic
      ? rotateAfterServed(
          candidates,
          rotationKeyOf,
          loadCaptureRotation(this.buffer.database, "claude_code"),
        )
      : candidates
