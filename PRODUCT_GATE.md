...
| G5 | Easy link-up: fresh Mac → numbers < 5 min, no hand-editing | source installer dry-run + strict read-only doctor proof (#107); lifecycle transaction source proof (#103) + packaged operator-command end-to-end proof (`proof:lifecycle-operator`): immutable self-pinned runtime, auto-rollback, preview-default uninstall/purge; fresh-Mac timing still requires `signal_verified` from a real token event; npm publication + live rollout pending | 🟡 installer/lifecycle safety automated through the packaged CLI; publication and live capture timing pending |
