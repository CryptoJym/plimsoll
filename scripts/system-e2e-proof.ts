...
async function main() {
  ...
  assert.equal(idle?.counters.rawEventWrites, 0);
  assert.equal(idle?.counters.rawEventRewrites, 0);
  assert.equal(idle?.counters.filesOpened, 0);
  assert.equal(idle?.counters.fileBytesRead, 0);
  assert.equal(idle?.counters.fullHistoryFileReads, 0);
