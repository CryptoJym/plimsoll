...
export function renderPrivacySpec(model: PrivacySpecModel): string {
  ...
  lines.push(`Each fact table also has a generated local \`retention_ms\` column: its timestamp converted to UTC epoch milliseconds, indexed together with the fact identity. The original timestamp remains stored unchanged where the fact contract permits offsets. The key and its index never leave the ledger. On upgrade SQLite builds the key indexes for existing rows in the same atomic migration as capacity enforcement; no partially ranked table is visible. Older collector writes continue to populate the generated key automatically. Row deletion removes the fact and its index entry together.`);
  lines.push(``);
  ...
  lines.push(``);
  lines.push(`Raw event retention and projection compaction do not remove these independent promoted facts. If admitting a child or retry would evict a required parent or retry target, the dependent is dropped and counted before any eviction; the retained ancestor stays. No orphan is retained. Drop reasons and totals live in the bounded, local \`runtime_fact_drops\` table; there are no per-fact retention logs.`);
  lines.push(``);
