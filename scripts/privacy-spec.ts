...
export function renderPrivacySpec(model: PrivacySpecModel): string {
  ...
  lines.push(`Each cap is a hard row bound. At a full table, the incoming fact competes with the oldest retained fact by UTC instant, then identity, regardless of how the producer spelled its timezone offset. If it ranks lower, the collector drops that table's fact without inserting or evicting a fact and increments the local \`outside_retention_window\` counter and coverage boundary; otherwise the write transaction evicts the oldest indexed row(s). Maintenance selects at most 256 oldest roots per table per pass, plus their dependent facts. Evicting an episode removes its descendants, attempts, and exposures together; evicting an attempt also removes its retry descendants. A late reference to an evicted episode or retry target is dropped and counted, never written as an orphan and never raised into capture. When prior eviction explains the missing reference, the lost attempt, child episode, or exposure also advances its table's coverage boundary.`);
  lines.push(`Each fact table has a stored local \`retention_ms\` key and a local \`retention_verified\` marker. Current writes set both from the fact schema and JavaScript \`Date.parse\`; the key is UTC epoch milliseconds indexed with the fact identity. Raw or older writers leave the marker empty even if SQLite supplied a provisional key. On current-version open, and before readback or retention if a raw row arrives meanwhile, the collector checks every unverified row against the fact schema and \`Date.parse\` in indexed batches, repairs any mismatched key, and only then enforces capacity. All startup work is one atomic migration, so a partially verified table cannot evict a valid fact. If a raw legacy row is invalid, the collector removes its whole fact graph and increments the bounded local \`invalid_retention_timestamp\` counter, not the capacity-eviction counter. Removed rows, including the invalid row and any valid dependents, advance the coverage boundary using their parseable timestamps, provisional keys, or the operation time when neither is available; future cutoffs are clamped to the operation time. The original timestamp remains stored unchanged where the fact contract permits offsets. The keys, markers, and indexes never leave the ledger; row deletion removes them together.`);
  lines.push(``);
  ...
  lines.push(`| \`learning_fact_table_state\` | Four local rows keyed by \`table_name\`, each with \`row_count\`, \`evicted_count\`, \`maintenance_needed\`, \`updated_at\`, \`loss_through_ms\` (exclusive coverage cutoff from lost facts or a conservative first-open fallback, with future values clamped on write or open), \`loss_evicted_count\`, and \`loss_drop_count\` (counter mirrors checked before claiming coverage). | 4 state rows | Atomic state/trigger initialization and missing-trigger reconciliation keep counts accurate. A status refresh reads these rows and bounded local \`runtime_fact_drops\` counts and last cap refusal to report window coverage; ledger purge removes them. |`);
  lines.push(``);
  lines.push(`Raw event retention and projection compaction do not remove these independent promoted facts. If admitting a child or retry would evict a required parent or retry target, the dependent is dropped with \`protected_reference_at_capacity\` before any eviction; the retained ancestor stays. No orphan is retained. Drop reasons and totals live in the bounded, local \`runtime_fact_drops\` table; there are no per-fact retention logs.`);
  lines.push(``);
