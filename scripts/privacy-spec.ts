...
export function renderPrivacySpec(model: PrivacySpecModel): string {
  ...
  lines.push(`Each fact table also has a stored local \`retention_ms\` column: its timestamp converted by JavaScript \`Date.parse\` to UTC epoch milliseconds, indexed together with the fact identity. The original timestamp remains stored unchanged where the fact contract permits offsets. The key and its index never leave the ledger. On upgrade, the collector backfills all missing keys and enforces capacity inside one atomic migration; no partially ranked table is visible. Older collector writes may leave an unsupported spelling's key empty, but the next current-version open repairs it before retention. If a raw legacy timestamp cannot be parsed, the collector removes its whole fact graph and increments the bounded local \`invalid_retention_timestamp\` counter. Row deletion removes the fact and its index entry together.`);
  lines.push(``);
