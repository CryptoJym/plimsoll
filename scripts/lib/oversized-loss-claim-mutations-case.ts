/** Two source regressions must be rejected by the real oversized claim proof.
...
const mutations = [
  {
    name: "lexical_first",
    before: "const {type, payloadType} = recordTypes(prefix);",
  ...
  },
  {
    ...
    before: 'return { kind: "unknown", usagePossible: true };',
    after: 'return { kind: "unknown", usagePossible: false };',
...
try {
  for (const mutation of mutations) {
    ...
    assert(source.includes(mutation.before),`missing mutation anchor: ${mutation.name}`);
