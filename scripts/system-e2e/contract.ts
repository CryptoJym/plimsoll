...
export const SUPPORT_NORMALIZATION_VERSION = 8 as const;
...
// The resource receipt keeps the raw directory work in evidence. These four
// bounded values are scheduling-sensitive, so the semantic digest records a
// stable placeholder after the receipt checks their limits.
const RESOURCE_VOLATILE_NUMBER_PATH =
  /^(?:root\.scenarios\[\d+\]\{id=bounded_generation_capture\}\.(?:counters\.(?:fileBytesRead|filesOpened|maintenanceRuns)|measurements\.(?:rssGrowthBytes|statusProbes|warmStatusP95Ms))|root\.scenarios\[\d+\]\{id=dashboard_projection_budget\}\.measurements\.generation|root\.scenarios\[\d+\]\{id=no_change_constant_work\}\.(?:counters\.(?:filesystemEntriesScanned|maintenanceRuns)|measurements\.(?:baselineCadences|maxCodexPendingMetadata|maxClaudePendingMetadata|maxAggregatePendingMetadata|filesystemEnumerationCalls|setupFilesystemEntriesScanned|unchangedFilesystemEntriesScanned)))$/;
