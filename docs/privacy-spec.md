...
Each fact table also has a generated local `retention_ms` column: its timestamp converted to UTC epoch milliseconds, indexed together with the fact identity. The original timestamp remains stored unchanged where the fact contract permits offsets. The key and its index never leave the ledger. On upgrade SQLite builds the key indexes for existing rows in the same atomic migration as capacity enforcement; no partially ranked table is visible. Older collector writes continue to populate the generated key automatically. Row deletion removes the fact and its index entry together.
