...
| `learning_fact_table_state` | Four local rows keyed by `table_name`, each with `row_count`, `evicted_count`, `maintenance_needed`, `updated_at`, `loss_through_ms` (one millisecond past the newest lost fact), `loss_evicted_count`, and `loss_drop_count` (counter mirrors checked before claiming coverage). | 4 state rows | Atomic state/trigger initialization and missing-trigger reconciliation keep counts accurate. A status refresh reads these rows and bounded local `runtime_fact_drops` counts and last cap refusal to report window coverage; ledger purge removes them. |
