# Freeze list after B0 round 7 (round 13, eco-6hoxj.164.4)

**As of:** 2026-09-26 MDT. Start point: the round-12 `FREEZE.md`, the lead's signatures (the eight round-9 "freezable now" items, and
**C4**), and the independent read of C1 after round 12 (`input/read-c1-r12/VERDICT.json`: **NOT_YET**; the whole-ledger merge confirmed
to close both round-11 counterexamples, the r11 fixture 60/69 red and r12 69/69 green, the reader's own PostgreSQL 17.10 probe agreeing;
C2, C3, C4, C5 and C8 byte-identical to round 11; two blocking items, no should-fix). A contract is **freezable** when its rule is stated
once, its fixture is green under the current rule, its pending test exists in the owning repository, and no open review finding touches
it. **Freezing is the lead's signature**, not B0's: this list says what is signed, what is ready for that signature and what still needs
a review round. Nothing is frozen by this document.
...
| **The joined-install scope of the pair and the null stamp before the first response (C4)** | CONTRACTS.md C4; ARCHITECTURE.md §5.2; `fixtures/b4_offline_rebind.py` case L; collector `actor-stamp.contract.ts`; cloud `actor-binding-stamp.contract.test.ts`. The read of C1/C4 passed it. Round 10 added only what its should-fixes asked. **Round 11 adds only a cross-reference** (a pre-re-join row delivered through a ledger the join could not link is held by C1 until the ledger is linked) **and one additive `/status` field** (`parkedRows`); the collector's promise the lead signed, keep the pair on rows already in the outbox at a re-join, is unchanged, and the judgment of such rows was always C1's. If the lead reads that cross-reference as a change to C4, the alternative is to strike it from C4 and let C1 alone carry the hold; every test binds C1. **Rounds 12 and 13 change nothing in C4** (the two reads confirmed its round-11 changes as additive and its text byte-identical since); C4's parenthesis counting the collector test's cases (12) is now one behind C1's list (13) and is left as it stands. |

## Ready after one more independent read of the round-13 text

| Contract | Where | State |
|---|---|---|
| **The actor-ownership predicate (C1) with the null-stamp exception** | CONTRACTS.md C1 (guarantee 5, "Serialization" and "Scope" round 11; "The link is the release" round 12; the retry of an aborted transaction in "Serialization" and the ledger on the 400 in "Outside the ledger" round 13; the C4 sentence and `/status` field round 11 added; C6); ARCHITECTURE.md §5.2, §11; MIGRATION.md; `fixtures/b4_offline_rebind.py` (r13 76/76; red under r6, r7, r8, r9, r10, r11 and r12); cloud `actor-binding-stamp.contract.test.ts` (16 cases), `actor-binding-stamp-postgres.contract.test.ts` (9 cases, with `fixtures/join-route-child.ts`), `schema-additions.contract.test.ts` (5 cases); collector `actor-stamp.contract.ts` (13 cases) | The read of the round-12 text confirmed the whole-ledger merge (both round-11 counterexamples closed: the r11 fixture 60/69, r12 69/69, and the reader's own PostgreSQL 17.10 probe) and said **not yet** for two items: a multi-pair sighting, locking installs in ascending id order, and the merge, locking the roots before the members, can deadlock, and the text said nothing about the transaction PostgreSQL aborts, so an aborted link did not release the parked rows; and the 400 refusal named no ledger while the release of a parked row compares ledgers, so the value the collector parks under had no wire provenance. Closed in this round with fixture checks red under round 12 and pending tests: **a C1 transaction PostgreSQL aborts is run again from its start, at most three attempts, nothing acknowledged before its commit, the exhausted retry a 503**; **the 400 names the ledger it judged**, the uploader's ledger the request was authorized with, and the collector parks under that wire value only. Both were reproduced on PostgreSQL 17.10 in both interleavings and shown closed (`checks/round13-sql-orderings.log`); the verdict had no should-fix (`CHANGES.md`). The design the four reads accepted (the pair, the sticky first-sighting fact, lock then read, the ledger key with the join's proof, `ON CONFLICT DO NOTHING` with a re-read, the cap, the refusal across ledgers with the hold, the whole-ledger merge into a canonical destination with the join's lock and the ledger on the response, the null-stamp exception, lifecycle-independent resolution, the erasure-only guard, retention until erasure) is unchanged. **Not listed as ready outright**: the retry rule and the refusal's ledger are new text no reviewer has read; one independent read of C1 round 13 is the remaining condition for signature. |

## Not freezable yet (needs a review round or a measurement)

| Contract | Why not |
|---|---|
| **The S1b runway numbers** as a gate that may start a live hold | The row widths (`F`, `T`, `S`, `SD`, `DR`, `RB`, `SEG`, `I`) are still column-width estimates and only Studio4's cardinalities are measured (its segment proxy is not: the census was not re-run in rounds 8-13). The fixture shows the rule overstates the runway when the true widths exceed 1.25 × the estimates (and does not self-correct), so no hold may start on any host before S2 has measured the widths on the Studio5 copy and B1's post-catch-up census has run on that host's copy (review r6 `firstWave`: "no live hold or S2"). |
...
1. C1 (the actor predicate as the pair fixed at its first sighting, the sighting locking first and reading everything under the lock,
   ...
   transaction retried from its start with nothing acknowledged before commit, the refusal naming the ledger it judged,
   lifecycle-independent resolution, the erasure-only guard; the null-stamp exception): ready **after one independent read of the
   round-13 text** (C1 "Serialization" and "Outside the ledger" round 13, the round-12 text the read confirmed, C6's surfaces).
...
## First-wave conditions from reviews r1 and r2 and the three reads of C1, and their state
...
- The read of C1 after round 12: its two blocking items are closed above (a deadlocked C1 transaction retried from its start, at most three attempts, nothing acknowledged before commit, the exhausted retry a 503; the 400 naming the ledger it judged and the collector parking under that wire value); it had no should-fix. Its notes stand: the pending state of every B6/B2a test is expected and is not an implementation proof.
