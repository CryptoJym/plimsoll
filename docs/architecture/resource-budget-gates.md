...
Poison continuation and the dashboard-projection budget now run through production seams. Only the two #81 cross-lane scenarios, `integrated_capture_projection_outbox` and `metadata_privacy_sentinels`, remain explicitly `not_wired`. The receipt therefore remains `gateReady: false`, and `--require-integrated` remains a required non-zero result until #81 wires them.
