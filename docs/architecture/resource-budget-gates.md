...
| Valuable OTLP usage/error/action/linkage fixture | exactly one sanitized admission per deterministic ID, one privacy-safe projection fact, one outbox enqueue; all production projection-row writes are measured | Admission <= 100 ms per fixture | Filtering must not trade CPU for silent data loss. |
...
- **#81 metadata privacy:** a second child and SQLite database carry synthetic prompt, response, tool-argument, path, repository-URL, email, credential, cookie, and multibyte sentinels. Full values and unique prefixes, plus the real operator-home path, are checked across live SQLite text, closed database/WAL bytes, upload bytes, stable malformed-request logs/responses, child stdout/stderr, the worker result, and the final resource receipt. Only counts and booleans enter proof output. Malformed JSON failures expose a stable error code, allowlisted error class, method, and route family; exception messages, stacks, bodies, raw URLs, and queries are excluded.
