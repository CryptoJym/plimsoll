...
Raw retention is now an independent, bounded maintenance pass. Each pass
visits at most its configured row budget, writes a payload-free
`raw_retention_receipts` row with reason `retention_window_elapsed`, and then
deletes the raw row. A configured delivery outbox protects an un-migrated
legacy row until its sanitized copy exists; after that point pending delivery
survives raw expiry. The outbox may therefore deliver an expired raw lineage,
but acknowledgement never sets `buffered_events.uploaded_at` when that raw row
is already gone. An unrecorded raw deletion remains a local privacy/integrity
failure. Local-only and `not_enrolled` ledgers have no migration protection and
expire old raw rows directly. Status reports retained, pending-delivery,
quarantined, expired, and not-inspected states separately; `not-inspected` is
used by doctor when no authenticated daemon snapshot is available.

SQLite retention is logical product erasure, not a forensic secure-delete
claim. A live WAL/SHM pair is part of the database state and must be handled as
one unit: copying the main `.sqlite` file alone while the collector is open is
unsupported, and doctor never opens SQLite. Supported lifecycle snapshots use
SQLite's online-backup API after the retention transaction, producing a
quiesced destination and removing stale destination `-wal`/`-shm` sidecars.
Pending sanitized delivery envelopes are intentionally visible as
`pendingDelivery` until acknowledgement or quarantine; expiry never presents
that recoverable product copy as uploaded.
